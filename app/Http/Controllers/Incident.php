...
class Incident extends Controller
{
    public function Index(Request $request)
    {
        ...
        // Terapkan semua filter ke $query terlebih dahulu
        ...
        // Clone query SETELAH semua filter diterapkan agar statistik mengikuti filter aktif
        $countQuery = clone $query;

        $data_incident = $query->orderBy('tanggal_lapor', 'desc')->paginate(20);

        $incident_log = [
            'data_incident' => $data_incident,
        ];

        // Hitung statistik prioritas berdasarkan data yang sudah difilter
        $total       = (clone $countQuery)->count();
        $total_p1    = (clone $countQuery)->where('prioritas', 'like', 'P1%')->count();
        $total_p2    = (clone $countQuery)->where('prioritas', 'like', 'P2%')->count();
        $total_close = (clone $countQuery)->where('status', 'Closed')->count();
